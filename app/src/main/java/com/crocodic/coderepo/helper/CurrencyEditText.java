...
public class CurrencyEditText extends android.support.v7.widget.AppCompatEditText {
     ...
     *
     ...
     <yourpackagename.CurrencyEditText
     app:locale="in_ID"
    ...
    public String getCurrencyTextWithoutDivider(){
        if (showSymbol())
            return getText().toString().replace(currencySymbol, "").replace(".","");
        else return getText().toString().replace(".","").replace(",","");
    }
}
